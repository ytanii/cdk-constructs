# TodoApp CDK Constructs

![Build & Release](https://github.com/todoapp-dev/cdk-constructs/workflows/Build%20&%20Release/badge.svg?branch=release)
![CodeQL](https://github.com/todoapp-dev/cdk-constructs/workflows/CodeQL/badge.svg)

[![CDK Constructs Maven Central](https://img.shields.io/maven-central/v/dev.todoapp/cdk-constructs.svg?label=CDK%20Constructs%20Maven%20Central)](https://search.maven.org/search?q=g:%22dev.todoapp%22%20AND%20a:%22cdk-constructs%22)
...
These constructs are explained in further detail in [our book](https://todoapp.dev).
...
```groovy
implementation('dev.todoapp:cdk-constructs:${latestVersion}')
```

The `latestVersion` is: [![](https://img.shields.io/maven-central/v/dev.todoapp/cdk-constructs.svg?label=)](https://search.maven.org/search?q=g:%22dev.todoapp%22%20AND%20a:%22cdk-constructs%22)
...
* **[DockerRepository](src/main/java/dev/todoapp/infrastructure/DockerRepository.java)**: a stack that contains a single ECR Docker repository and grants push and pull permissions to all users of the given account.
* **[Network](src/main/java/dev/todoapp/infrastructure/Network.java)**: creates a VPC with public and isolated subnets and a loadbalancer. Exposes parameters in the parameter store to be consumed by other constructs so they can be placed into that network.
* **[PostgresDatabase](src/main/java/dev/todoapp/infrastructure/PostgresDatabase.java)**: creates a PostgreSQL database in the isolated subnets of a given network. Requires a running `Network` stack (or at least the parameters it would expose in the SSM parameter store). Exposes the database connection parameters in the parameter store.
* **[JumpHost](src/main/java/dev/todoapp/infrastructure/JumpHost.java)**: creates an EC2 instance in a `Network`'s public subnet that has access to the PostgreSQL instance in a `PostgreSQLDatabase` stack. This EC2 instance can act as a jump host (aka bastion host) to connect to the database from your local machine.
* **[Service](src/main/java/dev/todoapp/infrastructure/Service.java)**: creates an ECS service that deploys a given Docker image into the public subnets of a given `Network`. Allows configuration of things like health check and environment variables.
* **[SpringBootApplicationStack](src/main/java/dev/todoapp/infrastructure/Service.java)**: a stack that combines the [Network](src/main/java/dev/todoapp/infrastructure/Network.java) and [Service](src/main/java/dev/todoapp/infrastructure/Service.java) constructs, configured for easy deployment of a Spring Boot Docker image.
